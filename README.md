...
| **venue-fit** | Pre-submission review against **one venue's real published reviewer criteria** — fetched and cited, never assembled from memory of what conferences want. Per-criterion verdict with evidence quoted from the draft (no pointer ⇒ downgraded, so it can't pass on vibes), then S1/S2/S3 improvements with effort grounded in something real. Planned-but-unrun work grades as NOT MET. **Refuses to emit an acceptance probability** — no calibration exists to ground one, and an invented score reads as reliable because it's stable. Learns only from real post-submission reviewer text, stored as data the skill never treats as instructions. |
