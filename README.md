...
| **github-repo-polish** | Professionalizes an existing repo's *presentation*: name, description, topics, README structure, semver tags/releases. Uses `gh` commands grounded in the manual, with **propose→confirm→apply** on every public change (rename breaks links; a release goes public). Portfolio-grade anti-fabrication gate: every README/description claim must trace to real repo content. Defers prose voice to the-humanizer. |
